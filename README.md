...
Ветка `feature/nanopi-manager` содержит реализацию итерации 03. Go-тесты проходят, Linux-бинарники собираются для ARM64 и AMD64. Сетевые изменения, автоматическое подтверждение доступности, обновление Manager, TUN и внешний VPN-inbound должны быть проверены вручную на NanoPi перед публикацией следующего релиза.
...
- одно действие «Применить» без технического diff: UI сам проверяет Manager по новому адресу и подтверждает рабочую конфигурацию;
- backup перед применением, невидимый пользователю rollback через 120 секунд при потере связи и постоянное отключение режима с возвратом к исходному состоянию;
- WAN default-drop; доступ к Manager и 3x-ui создаётся как обычное редактируемое TCP-правило;
...
- компактный адаптивный интерфейс Dense Rack с вертикальной навигацией;
- установка Docker, состояние daemon и список всех контейнеров с restart policy;
- отдельный раздел 3x-ui/Xray, доступный после запуска Docker, с атомарной сменой порта контейнера и его WAN-правила;
...
3. Нажать «Применить». Manager автоматически подтвердит конфигурацию после ответа по новому адресу; при потере связи устройство откатится само.
4. Установить Docker.
5. Установить 3x-ui.
