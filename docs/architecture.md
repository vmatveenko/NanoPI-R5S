...
7. Загруженная страница опрашивает Manager по новому порту и LAN-адресу. После первого успешного ответа она передаёт одноразовый токен и автоматически подтверждает ревизию; без ответа agent восстанавливает backup.
...
- Manager и 3x-ui по умолчанию закрыты на WAN. Их флажки создают, включают или выключают обычные пользовательские TCP-правила;
- при смене порта Manager или 3x-ui связанное правило переносится на новый порт, сохраняя описание и фильтр источников;
...
Порт панели хранится отдельно от конфигурации маршрутизатора. Его изменение, перезапуск контейнера и изменение nftables выполняются одной транзакцией; при ошибке compose или firewall восстанавливаются из backup.
